...
const redisClient = bluebird.promisifyAll(redis.createClient(REDIS_URL));
const vonageCredentials = {};

if (process.env.VONAGE_API_KEY && process.env.VONAGE_API_SECRET) {
  vonageCredentials.apiKey = process.env.VONAGE_API_KEY;
  vonageCredentials.apiSecret = process.env.VONAGE_API_SECRET;
}

if (process.env.VONAGE_APPLICATION_ID) {
  vonageCredentials.applicationId = process.env.VONAGE_APPLICATION_ID;
}

if (process.env.VONAGE_APPLICATION_PRIVATE_KEY) {
  vonageCredentials.privateKey = Buffer.from(
    process.env.VONAGE_APPLICATION_PRIVATE_KEY.replace(/\\n/g, '\n'),
    'utf-8'
  );
}

const vonage = new Vonage(vonageCredentials, { apiHost: 'messages-sandbox.nexmo.com' });
